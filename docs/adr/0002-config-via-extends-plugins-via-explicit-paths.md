...
- **`extends`** resolves configuration from an npm package via a `package.json` `exports` subpath (e.g. `extends: ["@sannajammehtry/biome-style/google-typescript"]`). This works well for `formatter` and `linter` settings.
...
Split distribution by layer, and keep both layers in **one** package (`@sannajammehtry/biome-style`), one subpath export per guide:

- **Config layer (formatter + built-in rules)** ships as a **shared config** at a subpath (`@sannajammehtry/biome-style/google-typescript`) and is consumed with a single `extends` entry. This shared config carries **no `plugins` key**.
- **Plugin layer** ships as **granular, per-rule `.grit` files**. Consumers reference them with **explicit `./node_modules/@sannajammehtry/biome-style/guides/google-typescript/plugins/*.grit` paths** in their own `plugins` array. The README documents the full copy-paste list.
